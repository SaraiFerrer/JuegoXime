class Game{
    ...
    addSprites(spriteGroup, numberOfSprites, spriteImage, scale, positions = []) {
      ...
      handleElements() {
        ...
        form.titleImg.class("Nombredeltitulo");
      ...
      play() {
        ...
        player.getObstaclesAtEnd();
    
        if (allPlayers !== undefined) {
          image(/*variable fondo,*/ 0, -height * 5, width, height * 6);
          ...
          if (player.positionY > finshLine) {
            gameState = 2;
            player.rank += 1;
            Player.updateObstaclesAtEnd(player.rank);
            player.update();
            this.showRank();
          }
      ...
      handleFriendsACollisionWithFriendsB(index) {
        ...
        if (index === 2) {
          if (friends[index - 1].collide(cars[0])) {
            if (this.leftKeyActive) {
              player.positionX += 100;
            } else {
              player.positionX -= 100;
